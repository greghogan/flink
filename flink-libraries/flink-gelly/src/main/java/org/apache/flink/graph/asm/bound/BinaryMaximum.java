...
public class BinaryMaximum<K extends Value, RK extends BinaryResult<K> & Comparable<RK>>
implements CustomUnaryOperation<RK, Tuple2<RK, ValueArray<K>>> {
	 ...
	 * @return this
	 */
	public BinaryMaximum<K, RK> setFlipInput(boolean flipInput) {
		this.flipInput = flipInput;
	...
	public DataSet<Tuple2<RK, ValueArray<K>>> createResult() {
		...
		DataSet<Tuple2<RK, ValueArray<K>>> resultWithEmptyArray = input
			.map(new Functions.MapToValueArray<K, RK>())
				.returns(returnType)
				.name("Map to ValueArray");

		return resultWithEmptyArray
			.groupBy("0.0")
			.reduce(new ReduceMaximum<K, RK>())
	 ...
	 */
	private static class ReduceMaximum<T, RT extends BinaryResult<T> & Comparable<RT>>
